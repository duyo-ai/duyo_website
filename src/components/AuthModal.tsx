...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
    ...
    switch (name) {
      ...
      case 'name':
        if (mode === 'signup' && !value.trim()) {
          errorMsg =
            lang === 'ko' ? '이름을 입력해주세요.' : 'Please enter your name.'
        }
        break
  ...
  const validateForm = () => {
    if (!formData.email) {
      setError(
        lang === 'ko' ? '이메일을 입력해주세요.' : 'Please enter your email.',
      )
      return false
    }
      ...
      if (mode === 'login') {
        ...
        try {
          ...
          const isGoogleOnly =
            ...
            info.providers.every((p: string) => p.toLowerCase() === 'google')
          if (isGoogleOnly) {
            setError(
              lang === 'ko'
        ...
        try {
          ...
          if (check?.ok && check.exists) {
            ...
            if (providers.includes('google')) {
              setError(
                lang === 'ko'
                  ? '이미 구글로 가입된 이메일입니다. 상단의 Google로 계속하기 버튼을 사용해주세요.'
                  : 'This email is already registered with Google. Please use "Continue with Google" above.',
              )
              return
            } else {
              setError(
                lang === 'ko'
                  ? '이미 가입된 이메일입니다. 로그인으로 이동해주세요.'
        ...
        if (data.ok) {
          ...
          if (data.error === 'EMAIL_ALREADY_EXISTS') {
            ...
            setError(
              data.error ||
                (lang === 'ko' ? '회원가입에 실패했습니다.' : 'Signup failed.'),
            )
          }
        }
      } else if (mode === 'forgot') {
        ...
        try {
          ...
          if (
            providers.length > 0 &&
            providers.every((p: string) => p.toLowerCase() === 'google')
          ) {
            setError(
              lang === 'ko'
                ? '이 이메일은 Google로 가입되어 비밀번호가 없습니다. 상단의 "Google로 계속하기" 버튼을 사용해 로그인해 주세요.'
  ...
  const getTitle = () => {
    switch (mode) {
      case 'login':
        return lang === 'ko' ? 'Cutple에 로그인' : 'Log in to Cutple'
      case 'signup':
        return lang === 'ko' ? 'Cutple 회원가입' : 'Create a Cutple account'
      case 'forgot':
        return lang === 'ko' ? '비밀번호 찾기' : 'Reset your password'
    }
  }
    ...
    switch (mode) {
      ...
      case 'signup':
        return lang === 'ko'
          ...
          : 'Create an account to start creating videos'
      case 'forgot':
        return lang === 'ko'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md transform transition-all">
        <div className="relative rounded-2xl border border-white/10 bg-black/40 p-6 shadow-2xl backdrop-blur-md">
          {/* 닫기 버튼 */}
          <button
            onClick={onClose}
          ...
          <div className="mb-8 text-center">
            {mode === 'forgot' && (
              <button
                ...
                className="group mb-4 inline-flex items-center gap-2 text-gray-400 transition-colors hover:text-white"
              >
          ...
          {mode !== 'forgot' && (
            <>
              <button
                ...
                className="mb-6 flex w-full items-center justify-center gap-3 rounded-xl bg-white px-4 py-3 font-semibold text-gray-900 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <svg
                  className={`h-5 w-5 ${googleLoading ? 'opacity-60' : ''}`}
                  viewBox="0 0 24 24"
                >
                  <path
                    fill="#4285F4"
              ...
              <div className="relative my-6 flex items-center justify-center">
                ...
                <span className="whitespace-nowrap bg-black/40 px-4 text-center text-xs text-gray-400">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div className="space-y-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full rounded-xl border bg-white/5 py-3 pl-12 pr-4 text-white placeholder-gray-400 transition-all focus:outline-none focus:ring-2 ${
                      fieldErrors.name
                        ? 'border-red-500 focus:ring-red-500'
                        : 'border-white/10 focus:border-transparent focus:ring-purple-500'
                    }`}
            ...
            <div className="space-y-1">
              ...
              {!fieldErrors.email &&
                emailCheck &&
                emailCheck.exists &&
                emailCheck.providers.length > 0 &&
                emailCheck.providers.every(
                  (p) => p.toLowerCase() === 'google',
                ) && (
              ...
              <div className="space-y-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={handleFieldBlur}
                    onInvalid={(e) => e.preventDefault()}
                    className={`w-full rounded-xl border bg-white/5 py-3 pl-12 pr-12 text-white placeholder-gray-400 transition-all focus:outline-none focus:ring-2 ${
            ...
            {mode === 'signup' && (
              <div className="flex items-start gap-3 pt-1 text-xs text-gray-300">
                <input
                  id="agreeTerms"
                  type="checkbox"
                  checked={agreeTerms}
                  onChange={(e) => setAgreeTerms(e.target.checked)}
                  className="rounded mt-0.5 h-4 w-4 border-white/20 bg-transparent"
                />
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full transform rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 py-3 font-semibold text-white transition-all hover:scale-[1.02] hover:from-purple-600 hover:to-indigo-700 disabled:transform-none disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isLoading
                ? lang === 'ko'
                  ? '처리 중...'
                ...
                : (() => {
                    switch (mode) {
                      case 'login':
                        return lang === 'ko' ? '로그인' : 'Log in'
                      case 'signup':
                        return lang === 'ko' ? '회원가입' : 'Sign up'
                      case 'forgot':
                        return lang === 'ko'
                          ? '재설정 링크 보내기'
                          : 'Send reset link'
